...
function importGoogleSheetData() {
  var publicSpreadsheetUrl = "https://docs.google.com/spreadsheets/d/1DOpXRRXV5EbM7OjMDWLmBvE_APAb3BtdsjVT8ptPj7Y/edit?usp=sharing";
...
function readFromCSV(center, radius) {
  return csvData.map(data => {
    let distance = null;
    let lat = importFrom === "sheet" ? data["Latitude"] : data[7],
      lng = importFrom === "sheet" ? data["Longitude"] : data[8];

    if (center && radius) {
      distance = calculateDistance({ lat: center.lat(), lng: center.lng() }, { lat: lat, lng: lng })
    }

    return {
      'id': importFrom === "sheet" ? data["ID"] : data[0],
      'name': importFrom === "sheet" ? data["Name"] : data[7],
      'address': importFrom === "sheet" ? data["Address"] : data[2],
      'lat': lat,
      'lng': lng,
      'distance': distance,
    }
